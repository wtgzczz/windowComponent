define(['widget','jquery','jqueryUI'],function(widget,$,$UI){
	...
	function Window(){
		this.cfg = {
			...
			text4AlertBtn:'确定',
			text4CancelBtn:'取消',
			handler4AlertBtn:null,  //确认按钮的回调
			handler4CloseBtn:null,  //关闭按钮的回调
	...
	Window.prototype = $.extend({},new widget.Widget(),{     /*通过合并所有的属性，成为一个最大的对象，赋值给prototype,window的实例就会具有所有的方法*/
		/*四种弹窗的入口函数*/
		alert:function(cfg){
		...
		bindUI:function(){
			var that = this;
			this.boudingBox.delegate(".window_alertBtn","click",function(){ /*点击确认按钮遍历并触发触发自定义事件*/
				that.fire('alert');
				that.destroy();
